...
int main(int argc, char** argv) {
    ...
    myfile2.open ("gesch.txt");
    
    std::random_device rd;
    std::mt19937 gen(rd());
    std::normal_distribution<> n(0,1);
 
int N=10000000;//Anzahl Zeitschritte
...
for(j=0;j<N;j++){
    if(j==(d2+1)*10000-1){
        d2=d2+1;
         for(l=0;l<100;l++){
	v[l]=v[l]-gam*v[l]*dt+F*dt-sin(x[l])*dt+sqrt(2*kT*gam*dt)*n(gen);
	x[l]=x[l]+v[l]*dt;
    }
   double sum4=0;
    for(a2=0;a2<100;a2++){
...
for(f=0;f<1000;f++){
myfile << f*100 << ' ' << defft[f] <<"\n";
}

for(g=0;g<1000;g++){
    myfile2 << g*100 << ' ' << vavt[g] << "\n";
}
    myfile.close();
    myfile2.close();
