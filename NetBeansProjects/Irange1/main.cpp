 ...
 * Author: richard
 *
 * Created on January 17, 2019, 12:13 AM
 */
...
int main(int argc, char** argv) {
    ...
    ofstream myfile2;
    ofstream myfile3;
    myfile.open ("deff32.txt");
    myfile2.open ("N32.txt");
    myfile3.open ("N232.txt");
...
for(j=0;j<N;j++){ 
    ...
    for(f=0;f<points;f++){
myfile << f*sample << " " << Deff[f] << "\n";
}
 myfile.close(); 
    for(f=0;f<points;f++){
myfile2 << f*sample << " " << Nav[f] << "\n";
}
 myfile2.close();     
 for(f=0;f<points;f++){
myfile3 << f*sample << " " << N2av[f] << "\n";
}
 myfile3.close(); 
