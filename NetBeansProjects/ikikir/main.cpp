 ...
 * Author: richard
 *
 * Created on March 18, 2019, 8:23 PM
 */
...
int main(int argc, char** argv) {
    ...
    double Nav,N2av,T;
    double v[runs],nf[runs],nfs[runs],vs[runs],uGe[Ivalues],uFano[Ivalues],uDeff[Ivalues],Deff,Deff2,deff[Ivalues],vav[Ivalues],Fano[Ivalues],vout[points],hout[points];
    int s;
    int p=0;
//for(s=0;s<Ivalues;s++){
    initd(nfs,runs);
...
for(j=0;j<Neq;j++){ 
    ...
    v[a]=I0*dt+vs[a]-gKir*ninf(vs[a],kh,vh)*(vs[a]-EK)*dt-gK*nfs[a]*(vs[a]-EK)*dt-sqrt(2*D*dt)*n(gen);
