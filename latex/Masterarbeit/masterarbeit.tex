...
\subsubsection{Spectrum and SNR}
...
\section{Other two-dimensional Neuron models}
...
\end{align*}
This leads to a slightly different phase space picture:
\begin{figure}[H]
	\centering
	...
	\label{anhopfnc}
\end{figure}
In contrast to the previous case, the system now not only consists of a stable limit cycle and a stable node but also an unstable limit cycle in between the two stable configurations. Upon increase of the bias current, the unstable limit cycle shrinks and makes the node lose stability.
