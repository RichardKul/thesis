...
In most of the computations, 500 neurons were simulated. Only for $D=0.25$, the number of neurons was set to 50 so that there would be a higher number of transitions in a single voltage curve. In all cases, the timestep was 0.5 $\mu$s. The total measurement time for a single neuron after an equilibration period between $2\cdot10^2$ s and $2\cdot10^3$s ranged from $10^3$ s for high noise intensities to $4\cdot 10^4$ s for the lowest noise intensities.
\subsection{Count statistics}
Firstly, the effective diffusion coefficient $D_{eff}$, the fano factor $F$ and the average firing rate $r$ could be obtained from the spike counts of the simulated neurons. \\
