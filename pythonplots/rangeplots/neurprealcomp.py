...
ivalues=20
Dcomp=[35,45]
datecomp='realfast19mtf'
lcomp=len(Dcomp)
veccomp=np.zeros((lcomp,ivalues))
vecxcomp=np.zeros((lcomp,ivalues))
ll=0

offset=np.zeros(lcomp,dtype=int)
for x in Dcomp:
	col,colx=[],[]	
	for y in range(istart,istart+ivalues):
		file=open('/home/richard/outhome/d%s%d%d.txt' % (datecomp,x,y),"r")
		for k in file:
			row=k.split()
			colx.append(float(row[0]))
			col.append(float(row[1]))
		if len(col)+offset[ll]<y-istart+1:
			offset[ll]=offset[ll]+1
	colxacomp=np.array(colx)
	colacomp=np.array(col)
	for z in range(0,ivalues-offset[ll]):
		veccomp[ll][z]=colacomp[z]*timefac
		vecxcomp[ll][z]=colxacomp[z]
	ll=ll+1
...
for n in range(l1+lvar,l1+lvar+l0):
	plt.plot(colxa0,vec[n,:],colorv[n],label='D=%.2f,old' %(D0[n-lvar-l1]/100))
for n in range(l1+lvar+l0,ltot):
	plt.plot(colxa,vec[n,:],colorv[n],label='D=%.2f,old' %(D[n-lvar-l1-l0]/100))
for n in range(0,lcomp):
	nl=round(ivalues-offset[n])
	plt.plot(vecxcomp[n,0:nl],veccomp[n,0:nl],label='D=%.2f,new' %(Dcomp[n]/100))
...
plt.savefig('dneurcomp%s.pdf' %datefull)
...
for x in Dcomp:
	col,colx=[],[]	
	for y in range(istart,istart+ivalues):
		file=open('/home/richard/outhome/f%s%d%d.txt' % (datecomp,x,y),"r")
		for k in file:
			row=k.split()
			colx.append(float(row[0]))
			col.append(float(row[1]))
...
#order = [1,0,2,3]
#plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order])
plt.savefig('fneurcomp%s.pdf' %datefull)
...
for x in Dcomp:
	col,colx=[],[]	
	for y in range(istart,istart+ivalues):
		file=open('/home/richard/outhome/g%s%d%d.txt' % (datecomp,x,y),"r")
		for k in file:
			row=k.split()
			colx.append(float(row[0]))
			col.append(float(row[1]))
...
#order = [2,1,3,4,0]
#plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order])
plt.savefig('gneurcomp%s.pdf' %datefull)
