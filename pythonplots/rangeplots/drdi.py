...
for x in [20]:
	...
	for z in range(0,20):
		vec[ii][z]=cola[z]
	for n in range(0,19):
		deffmitt[ii][n]=(cola[n+1]+cola[n])/2
	ii=ii+1
...
plt.legend()
plt.savefig('dneurp29m5.pdf')

plt.figure()
plt.ylabel('(dr/dI)^2/$D_{eff}$')
plt.yscale('log')
plt.plot(xsmitt,gdiff[0,:]**2/deffmitt[0,:],label='D=2')
plt.plot(xsmitt,gdiff[1,:]**2/deffmitt[1,:],label='D=2.5')
plt.plot(xsmitt,gdiff[2,:]**2/deffmitt[2,:],label='D=3')
plt.plot(xsmitt,gdiff[3,:]**2/deffmitt[3,:],label='D=4')
plt.plot(xsmitt,gdiff[4,:]**2/deffmitt[4,:],label='D=5')
#plt.plot(xs,vec[4,:],label='D=2')
#plt.plot(xs,vec[5,:],label='D=3')
#plt.plot(xs,vec[6,:],label='D=4')

plt.legend()
plt.savefig('snrapp29m5.pdf')
...
for x in [20]:
	col=[]	
	for y in range(1,21):
		file=open('/home/richard/outhome/f16m%d%d.txt' % (x,y),"r")
		for k in file:
			row=k.split()
			col.append(float(row[1]))
	cola=np.array(col)
