...
#plt.xlim(4*10**(-4),100)
	bx.plot(ax[0:axmax]*timefac,ay[0:axmax]*ax[0]*timefac,'blue')#,label='Simulation')
	bx.plot(ax[l-11:l]*timefac,ay[l-11:l]*ax[0]*timefac,'blue')
	bx.plot(ax2[10:-1]*timefac,ay2[10:-1]*ax2[0]*timefac,'blue')#,label='Simulation')
	...
	bx.plot(ax[0:axmax]*timefac,r*np.ones(axmax)*timefac,'g',label='overall firing rate')
	bx.plot(ax2[0:l2]*timefac,r*np.ones(l2)*timefac,'g')
	#bx.plot(ax[0:axmax]*timefac,2*d*np.ones(axmax)*timefac,'y',label='$2D_{eff}$')
	#bx.plot(ax2[0:l2]*timefac,2*d*np.ones(l2)*timefac,'y')
	#bx.plot(ax[0:axmax]*timefac,5200*np.ones(axmax),'r--',label='background')
	#bx.plot(ax2[0:l2]*timefac,5200*np.ones(l2),'r--')
	bx.plot(ax[0:axmax]*timefac,3*np.ones(axmax),'r--',label='background')
	bx.plot(ax2[0:l2]*timefac,3*np.ones(l2),'r--')
	...
	#plt.plot(10**(-4),2*dv*timefac,'bx')
	plt.arrow(0.05, 0.302, -0.05, 0,transform=bx.transAxes, length_includes_head=True,head_width=0.01,head_length=0.03)
...
#plt.plot(sax2,say2/T2,label='e6')
	...
	#bx.set_yticks(list(bx.get_yticks()) +[2*d*timefac])
	plt.tight_layout()
	bx.spines['right'].set_visible(False)
	bx.spines['top'].set_visible(False)
	plt.savefig('specpaper7.pdf')
