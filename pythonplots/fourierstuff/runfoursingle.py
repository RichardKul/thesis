...
for z in range(istart,istart+ivalues):
	file=open('/home/richard/cppworkshop/spike%s%d%d.txt' %(date,D,z),"r")
	...
	#	ax2[l]=ax[l]
	#	ay2[l]=ay[l]
	param=open('/home/richard/cppworkshop/param%s%d%d.txt' %(date,D,z),"r")
	ll=0
	name,value=[],[]
	for k in param:
		row=k.split()
...
#plt.xlim(4*10**(-4),100)
	plt.plot(ax,ay/T,label='Simulation')
