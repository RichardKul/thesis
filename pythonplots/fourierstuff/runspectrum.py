...
for z in range(0,datapoints):
	...
	if current>last_entry:
		...
		points_act=z
		break
x_act=np.zeros(points_act)
for z2 in range(0,points_act):
	x_act[z2]=ax[z2]

for ii in range(0,fpoints):
	f=3*ii/fpoints
	arr=np.zeros(points_act,dtype=np.complex_)
	for jj in range(0,points_act):
		arr[jj]=exp(f,ax[jj])
	S[ii]=(np.abs(np.sum(arr))**2)/T	
