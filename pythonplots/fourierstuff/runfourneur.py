...
#omega=np.arange(0,length)*2*np.pi/T
	...
	plt.suptitle('I=%.2f, D=%.2f' %(I0+z*di,D/100))
...
#plt.xlim(4*10**(-4),100)
	plt.plot(ax3/T*timefac,ay2/T*timefac,label='Simulation')
	plt.plot(ax3/T*timefac,r*np.ones(length)*timefac,label='average firing rate')
...
#plt.plot(sax2,say2/T2,label='e6')
	...
	plt.savefig('inapikneur%sfourierD=%.2fI=%.2f.pdf' %(date,D/100,I0+z*di))	
