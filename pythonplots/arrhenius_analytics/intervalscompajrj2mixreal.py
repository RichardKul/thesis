...
#plt.plot(xs,breltime[3,:],label='D=4,equilibrium')
	...
	params[3][k2]=popt[1]
	plt.legend()
	plt.savefig('arrheniustot%sfit%d.pdf' %(date1,k2))
	eqfile = open('param%s%d.txt' % (date1,k2),'w')
	for k3 in range(0,4): 
		eqfile.write('%.6f\n'%params[k3][k2]) 
	eqfile.close() 
	ratefile = open('rate%s%d.txt' %(date1,k2),'w')
	for k4 in range(0,3):
		ratefile.write('%.6f\n'%btottime[k4][k2])
	for k4 in range(0,3):  
		ratefile.write('%.6f\n'%eqtottime[k4][k2]) 
	...
	plt.savefig('cortime%s%d.pdf' %(date1,k2))
plt.figure()
xold=np.arange(-8+istart,-8+istart+ivalues)*0.025
...
plt.savefig('altcortime%s.pdf' %date1)
...
plt.legend()
plt.savefig('prefac%s.pdf' %date1)
plt.figure()
plt.xlabel('bias current')
...
plt.legend()
plt.savefig('barrier2%s.pdf' %date1)
eqfile3 = open('barrierex%s.txt' %date1,'w')
...
params2[5]=popt[0]
eqfile2 = open('parambarrier%s.txt' %date1,'w')
for k4 in range(0,6): 
	eqfile2.write('%.6f\n'%params2[k4]) 
eqfile2.close() 

t=np.arange(0.05,0.175,0.001)
plt.figure()
plt.xlabel('bias current')
plt.ylabel('potential barrier')
...
plt.legend()
plt.savefig('barriercomp%s.pdf' %date1)
