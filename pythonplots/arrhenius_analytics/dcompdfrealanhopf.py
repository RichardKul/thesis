...
for k2 in range(0,ivalues):
	x=[]
	ratefile = open('rate%s%d.txt' %(date3+date1,k2),'r')
	for k4 in ratefile:
		row=k4.split()
...
params2[2]=popt[0]
params2[3]=popt[1]
eqfile2 = open('paramsdfnew%s.txt'%(date3+date1),'w')
for k4 in range(0,2): 
	eqfile2.write('%.6f\n'%params2[k4]) 
eqfile2.write('%.6f\n'%rbte) 
for k4 in range(2,4): 
...
paramfile = open('parambarrier%s.txt' %(date3+date1),'r')
xx=[]
for k4 in paramfile:
	row=k4.split()
...
for k2 in range(0,ivalues):
    x=[]
    ratefile = open('/home/richard/mastergit/pythonplots/arrhenius_analytics/param%s%d.txt' %(date3+date1,k2),'r')
    for k4 in ratefile:
        row=k4.split()
        x.append(float(row[0]))
    ax=np.array(x)
