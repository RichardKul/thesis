...
date=['realfast9acoarsetf','realfast23mtf','realfast11jjem2st','realfast19mtf','realfast11jjem2st']
D=[30,35,40,45,50]
l=len(D)
istart=1
ivalues=20
...
for m in range(0,l):
	...
	for y in range(istart,istart+ivalues):
		...
		plt.yscale('log')
		plt.title("running intervals at $I=%.2f$, $D=%.2f$" %((-5+y)*0.02,x/100), fontsize=22)
		plt.savefig('bnewdistajrj2%s%d%df.pdf' %(date[m],x,y))
		plt.figure()
		plt.xlabel('interval length [s]')
		plt.ylabel('number of intervals')
		plt.hist(inteqt/1000, bins=50)
		plt.yscale('log')
		plt.title("equilibrium intervals at $I=%.2f$, $D=%.2f$" %((-5+y)*0.02,x/100), fontsize=22)
		plt.savefig('eqnewdistajrj2%s%d%d.pdf' %(date[m],x,y))
		eqtot=np.sum(inteqt)
		btot=np.sum(intbt)
...
cvbfile = open('cvb%s.txt' % (date[0]+date[1]),'w')
for kh in range(0,l): 
	for kl in range(0,ivalues): 
		cvbfile.write('%.6f '%cvb[kh][kl]) 
	cvbfile.write('\n')
cvbfile.close() 
cvefile = open('cve%s.txt' % (date[0]+date[1]),'w')
for kh in range(0,l): 
	for kl in range(0,ivalues): 
...
intdelayfile=open('intdelay%s.txt'%(date[0]+date[1]),'w')
for n in range(0,l):
...
chifile=open('rsquare%s.txt' %(date[0]+date[1]),'w')
...
if l > 1:
	for k2 in range(0,ivalues):
		...
		#plt.yscale('log')
		plt.plot(xs,np.log(timefac/btottime[:,k2]),'bo',label='sp. to eq.')
#plt.plot(xs,breltime[1,:],label='D=3,burst')
#plt.plot(xs,breltime[2,:],label='D=4,burst')
#plt.plot(xs,eqreltime[3,:],label='D=4,burst')
		plt.plot(xs,np.log(timefac/eqtottime[:,k2]),'ro',label='eq. to sp.')
#plt.plot(xs,eqreltime[1,:],label='D=3,equilibrium')
#plt.plot(xs,eqreltime[2,:],label='D=4,equilibrium')
#plt.plot(xs,breltime[3,:],label='D=4,equilibrium')
		...
		plt.tight_layout()
		plt.savefig('arrheniustotbig%sfit%dfln.pdf' %(date[0]+date[1],k2))
...
plt.legend()
plt.savefig('barrier2big%sf.pdf' %(date[0]+date[1]))
...
plt.legend()
plt.savefig('barriercompbig%sf.pdf' %(date[0]+date[1]))
