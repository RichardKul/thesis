...
for x in D1:
	for y in range(istart,istart+ivalues):
		...
		file=open('/home/richard/outhome/timenew%s%d%d.txt' % (date1,x,y),"r")
		for k in file:
			row=k.split()
		...
		param=open('/home/richard/outhome/param%s%d%d.txt' %(date1,x,y),"r")
		ll=0
		name,value=[],[]
		for k in param:
			row=k.split()
		...
		plt.yscale('log')
		plt.title("sp. intervals, $I=%.2f$, $D=%.2f$" %(43+0.25*y,x/100), fontsize=22)
		plt.tight_layout()
