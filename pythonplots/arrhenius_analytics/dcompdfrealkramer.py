...
for k2 in range(0,ivalues):
	...
	params[1][k2]=popt[1]
	params[2][k2]=popt[2]
	popt,pcov = curve_fit(func, xs, eqtob[:,k2],bounds=((-np.inf,0,0), (np.inf,100,1)))
	params[3][k2]=popt[0]
	params[4][k2]=popt[1]
	params[5][k2]=popt[2]

eqfile2 = open('paramskramer%s.txt'%(date1+date2),'w')
...
def rred(r0,U,D,alpha):
	return r0*(D**alpha)*np.exp(-U/D)
def deffred(rp,up,alphap,rm,um,alpham,v,D,):
	return (v**2*rred(rp,up,D,alphap)*rred(rm,um,D,alpham))/((rred(rp,up,D,alphap)+rred(rm,um,D,alpham))**3)
def vred(rp,up,alphap,rm,um,alpham,v,D):
	return v*rred(rm,um,D,alpham)/(rred(rp,up,D,alphap)+rred(rm,um,D,alpham))
def fanored(rp,up,alphap,rm,um,alpham,v,D):
	return 2*deffred(rp,up,alphap,rm,um,alpham,v,D)/vred(rp,up,alphap,rm,um,alpham,v,D)
...
for n in range(0,l):
	if n!=2:
		...
		plt.plot(xs,vec[n,:],colorv[n]+'o',label='D=%.2f$^*$' %(Da[n]/100))
for n in range(0,l):
	plt.plot((pv-4)*0.02,deffred(params[0][pv],params[1][pv],params[2][pv],params[3][pv],params[4][pv],params[5][pv],vvec[pv],Da[n]/100),colorv[n],label='D=%.2f'%(Da[n]/100))
...
plt.savefig('dcompdfkramer%s.pdf' %(date1+date2))
...
for n in range(0,l):
	if n!=2:
		...
		plt.plot(xs,g[n,:],colorv[n]+'o',label='D=%.2f$^*$' %(Da[n]/100))
for n in range(0,l):
	plt.plot((pv-4)*0.02,vred(params[0][pv],params[1][pv],params[2][pv],params[3][pv],params[4][pv],params[5][pv],vvec[pv],Da[n]/100),colorv[n],label='D=%.2f'%(Da[n]/100))
#plt.plot(xs,vec[4,:],label='D=2')
#plt.plot(xs,vec[5,:],label='D=3')
#plt.plot(xs,vec[6,:],label='D=4')
...
order = [2,3,0,1]
plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order])
plt.savefig('gcompdfkramer%s.pdf' %(date1+date2))
...
for n in range(0,l):
	if n!=2:
		...
		plt.plot(xs,fano[n,:],colorv[n]+'o',label='D=%.2f$^*$' %(Da[n]/100))
for n in range(0,l):
	plt.plot((pv-4)*0.02,fanored(params[0][pv],params[1][pv],params[2][pv],params[3][pv],params[4][pv],params[5][pv],vvec[pv],Da[n]/100),colorv[n],label='D=%.2f'%(Da[n]/100))
#plt.plot(xs,vec[4,:],label='D=2')
#plt.plot(xs,vec[5,:],label='D=3')
#plt.plot(xs,vec[6,:],label='D=4')
...
order = [2,3,0,1]
plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order])
plt.savefig('fcompdfkramer%s.pdf' %(date1+date2))
...
plt.plot(xs,params[4,:],colorv[1]+'o',label='eq to burst')
plt.plot(xs,2*params[1,:],colorv[2]+'o',label='2x burst to eq')
plt.plot(xs,2*params[4,:],colorv[3]+'o',label='2x eq to burst')
plt.legend()
plt.savefig('barriercompkramer%s.pdf' %(date1+date2))
