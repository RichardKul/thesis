...
for x in [30]:
	for y in range(1,21):
		...
		file=open('/home/richard/outhome/time18a%d%d.txt' % (x,y),"r")
		for k in file:
			row=k.split()
		...
		if statea[0]<0.5:
			...
			for z in range(0,5000):
				if avaluesa[2*z+1]*repetitions+jrvalues[2*z+1]+j2valuesa[2*z+1]/Ndiff>avaluesa[2*z]*repetitions+jrvalues[2*z]+j2valuesa[2*z]/Ndiff:
					inteq[z]=(((avaluesa[2*z+1]-avaluesa[2*z])*repetitions+jrvalues[2*z+1]-jrvalues[2*z])*Ndiff+j2valuesa[2*z+1]-j2valuesa[2*z])*dt
					counteq=counteq+1
				else:
					break	
			...
			for z in range(0,4999):
				if avaluesa[2*z+2]*repetitions+jrvalues[2*z+2]+j2valuesa[2*z+2]/Ndiff>avaluesa[2*z+1]*repetitions+jrvalues[2*z+1]+j2valuesa[2*z+1]/Ndiff:
					intb[z]=(((avaluesa[2*z+2]-avaluesa[2*z+1])*repetitions+jrvalues[2*z+2]-jrvalues[2*z+1])*Ndiff+j2valuesa[2*z+2]-j2valuesa[2*z+1])*dt
					countb=countb+1
				else:
					break
			...
			for z in range(0,5000):
				if avaluesa[2*z+1]*repetitions+jrvalues[2*z+1]+j2valuesa[2*z+1]/Ndiff>avaluesa[2*z]*repetitions+jrvalues[2*z]+j2valuesa[2*z]/Ndiff:
					intb[z]=(((avaluesa[2*z+1]-avaluesa[2*z])*repetitions+jrvalues[2*z+1]-jrvalues[2*z])*Ndiff+j2valuesa[2*z+1]-j2valuesa[2*z])*dt
					countb=countb+1
				else:
					break	
			intbt=np.zeros(countb)
			...
			for z in range(0,4999):
				if avaluesa[2*z+2]*repetitions+jrvalues[2*z+2]+j2valuesa[2*z+2]/Ndiff>avaluesa[2*z+1]*repetitions+jrvalues[2*z+1]+j2valuesa[2*z+1]/Ndiff:
					inteq[z]=(((avaluesa[2*z+2]-avaluesa[2*z+1])*repetitions+jrvalues[2*z+2]-jrvalues[2*z+1])*Ndiff+j2valuesa[2*z+2]-j2valuesa[2*z+1])*dt
					counteq=counteq+1
				else:
					break
			inteqt=np.zeros(counteq)
...
plt.legend()
plt.savefig('inapiktimes183.pdf')
